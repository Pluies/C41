...
void C41Window::create_objects()
{
     ...
     add_subwindow(skip_comp = new C41Enable(plugin, &plugin->config.skip_comp, x, y, _("Compute negfix values")));
     y += 20;
...
void C41Effect::render_gui(void* data)
{
     if(thread)
     {
          if(load_configuration()){
               ...
               thread->window->skip_comp->update(config.skip_comp);
...
int C41Effect::load_defaults()
{
     ...
     config.skip_comp = defaults->get("SKIP_COMP", config.skip_comp);
...
int C41Effect::save_defaults()
{
     ...
     defaults->update("SKIP_COMP", config.skip_comp);
...
void C41Effect::save_data(KeyFrame *keyframe)
{
     ...
     output.tag.set_property("SKIP_COMP", config.skip_comp);
...
void C41Effect::read_data(KeyFrame *keyframe)
{
     ...
     while(!input.read_tag())
     {
          if(input.tag.title_is("C41"))
          {
               ...
               config.skip_comp = input.tag.get_property("SKIP_COMP", config.skip_comp);
